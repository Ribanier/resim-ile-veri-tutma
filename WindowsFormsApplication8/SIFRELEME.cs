...
namespace WindowsFormsApplication8
{
    public partial class SIFRELEME : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                baglan();
                Bitmap bpm1 = new Bitmap(pictureBox1.Image);//resmimizi bitmape atadık               

                string[,] bpmpixeli = new string[bpm1.Height, bpm1.Width];
                //                                    satır   sütun      
                ...
                OleDbCommand cmd = new OleDbCommand("select isim from verikayit where isim ='" + sifrelenecek_Ad + "'", blnt);
                OleDbDataReader rd = cmd.ExecuteReader();
                if (rd.Read())
                {
                    MessageBox.Show(rd["isim"].ToString() + " Adındaki çözme kodu daha önce alınmış lütfen başka bir çözme kodu giriniz..." + blnt);
                }
                else
                {
                    ...
                    for (int i = 0; i < harfler.Length ; i++)
                    {
                        do
                        {
                            int a = rnd.Next(0, bpm1.Width);
                            bpmpixeli[i, a] = harfler[i].ToString();
                            //satır sütun
                            // bpm1.SetPixel(a, i, Color.Black);
                            //satır sütun 


                            r = bpm1.GetPixel(a, i);//rengimizi aldık

                            int A = r.A; //rengimizin alfa kanalını aldık

                            int R = r.R; R = R == 255 ? R - 1 : R + 1; //renk kodları 0-255 arası oldugu için hata almamak 
                            //için kontrol ettirdik renk kodunu 1 arttırdık ve ya azalttık rengimizin kırmızı RED tonu

                            int G = r.G; G = G == 255 ? G - 1 : G + 1;//rengimizin yeşil tonu

                            int B = r.B; B = B == 255 ? B - 1 : B + 1; // rengimizin mavi tonunu aldık 
                            
                            r = Color.FromArgb(A, R, G, B);//düzenlediğimiz renk kodunu tekrark color sınıfından r ye atadık
                           
                            bpm1.SetPixel(a, i, r);
                       //     bpm1.SetPixel(a, i, Color.White);
                            pictureBox1.Image = bpm1; asd++;
        ...
        /*private void button5_Click(object sender, EventArgs e)
        {
            ...
            if (sonuç == DialogResult.OK)
            {
                ...
                MessageBox.Show("Çözme kodunuzu not etmeyi unutmayınız...");
            }
        }*/
